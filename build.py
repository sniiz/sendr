...
notify_desktop.notify(f'android build {build_id} complete in {time.time() - start} seconds',
                      title='sendr', activate='com.microsoft.VSCode')
notify_phone().send(
    message=f'android build {build_id} complete in {time.time() - start} seconds')
...
notify_desktop.notify(f'ios build {build_id} complete in {time.time() - start} seconds',
                      title='sendr', activate='com.microsoft.VSCode')
notify_phone().send(
    message=f'ios build {build_id} complete in {time.time() - start} seconds')
